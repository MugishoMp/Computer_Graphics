...
void PreDraw() {
    ...
    GLint offset_x_location = glGetUniformLocation(G_GRAPHICS_PIPELINE_SHADER_PROGRAM, "u_offset_x");
    GLint offset_y_location = glGetUniformLocation(G_GRAPHICS_PIPELINE_SHADER_PROGRAM, "u_offset_y");

    if (offset_x_location >= 0) {
        glUniform1f(offset_x_location, G_U_OFFSET_X);
    } else {
        std::cout << "Could nt find offset_x" << std::endl;
    }

    if (offset_y_location >= 0) {
        glUniform1f(offset_y_location, G_U_OFFSET_Y);
    } else {
        std::cout << "Could nt find offset_y" << std::endl;
    }
}
...
void Draw() {
    ...
    // traignles says basicaly to sample three element sat a time
