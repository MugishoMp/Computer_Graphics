...
void VertexSpecifiation() {
    ...
    const std::vector<GLfloat> vertexData {
        ...
        // 0 - vertex
        -1.0f, -1.0f, 0.0f, // left vertex 1 position
        1.0f, 0.0f, 0.0f, // vertex 1 color
        // 1 - vertex
        1.0f,  -1.0f, 0.0f, // right vertex 2 position
        0.0f, 1.0f, 0.0f, // vertex 2 color
        // 2 - vertex
        -1.0f,   1.0f, 0.0f, // top vertex 3 position
        0.0f, 0.0f, 1.0f, // vertex 3 color
        // 3 - vertex
        1.0f, 1.0f, 0.0f, // top right vertex 1 position
        1.0f, 0.0f, 0.0f, // vertex 1 color
...
void Input() {
    ...
    if (state[SDL_SCANCODE_UP]) {
        G_U_OFFSET_Z += 0.001f;
        std::cout << "offset z: " << G_U_OFFSET_Z << std::endl;
    }
...
void PreDraw() {
    ...
    // Translation matrix
    glm::mat4 model = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 0.0f, G_U_OFFSET_Z));
    ...
    glm::mat4 perspective = glm::perspective(glm::radians(45.0f), 
        ...
        0.1f,
        10.0f);
    GLint perspective_matrix_location = glGetUniformLocation(G_GRAPHICS_PIPELINE_SHADER_PROGRAM, "u_perspective_matrix");

    if (perspective_matrix_location >= 0) {
        glUniformMatrix4fv(
    ...
    GLint resolutionLocation = glGetUniformLocation(G_GRAPHICS_PIPELINE_SHADER_PROGRAM, "u_resolution");
    glUniform2f(resolutionLocation, G_WIDTH, G_HEIGHT);
